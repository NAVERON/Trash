...
public class AutoNavigation extends Application {
    ...
    public static List<Ship> ships = new LinkedList<>();
    ...
    public void onUpdate(){
        ...
        for(int k = 0; k < ships.size(); k++){
            Navigator temp = ships.get(k);
            tracks.getChildren().add(new Circle(temp.getPosition().getX(), temp.getPosition().getY(), 2, Color.BLUE));
        }
            ...
            if (!ships.isEmpty()) {
                ...
                if (e.getCode() == KeyCode.LEFT) {
                    ...
                    temp.setVelocity( temp.getVelocity().add(
                        new Point2D( Math.cos(Math.toRadians(temp.getAngle())), Math.sin(Math.toRadians(temp.getAngle())) )
                    ) );
                } else if(e.getCode() == KeyCode.DOWN){  //减速无法减速至停止
    ...
    public void saveFunction(List<Ship> ships){
        ...
        for(int i = 0; i < ships.size(); i++){
            Saving.save(ships.get(i).getHistorys(), ships.get(i).getId());
        }
        for(int i = 0; i < ships.size(); i++){
