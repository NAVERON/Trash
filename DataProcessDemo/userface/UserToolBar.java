...
public class UserToolBar extends JPanel{
	...
	//面板组件 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	private JComboBox<String> shipName = new JComboBox<>();
	private JComboBox<String> shipMmsi = new JComboBox<>();
	...
	public UserToolBar(List<StateReport> statereports, List<PosReport> posreports) {
		...
		DateChooser kaishi = new DateChooser(this);
		add(kaishi);
		/*startDate.setColumns(20);
		add(startDate);*/
		DateChooser jieshu = new DateChooser(this);
		add(jieshu);
	...
	public List<PosReport> getselectPos(String mmsi, Date start, Date end){  //根据日期从数据筛选
		List<PosReport> selectPos = new ArrayList<>();
		PosReport temp = null;
