...
import data.AccessData;
import data.PosDAO;
import data.PosReport;
import data.StateDAO;
import data.StateReport;

public class DemoMain extends JFrame{
	
	private static final long serialVersionUID = 5598601039782040834L;
	...
	public static void main(String[] args) {
		...
		//从文件中/或者其他网络资源中获取数据
		List<StateReport> statereports = access.getStateReport();
		List<PosReport> posreports = access.getPosReport();
		...
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				new DemoMain(statereports, posreports);  //DemoMain demoMain = 
			}
		});
	}
	...
	private void initialInterface(){
		...
		/*************************左边相应的船舶信息************************************/
		//左边船舶信息栏
		//ShipTableModel infoData = new ShipTableModel(statereports.get(1));  //数据需要通过这个继承类传入，并更新表格
		...
		shipInfo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				String getMmsi = (String) new AllShipModel(statereports).getValueAt(row, 0);
				
				List<PosReport> selected = new ArrayList<>();
